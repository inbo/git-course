...
* tu abbia [git](https://git-scm.com/) installato sul computer e puntanto sull'eseguibile `git.exe`. (~ 3 min.)
...
L'ultima versione di `git` disponibile è la `r read_html("https://git-scm.com/") %>%  html_nodes(css = ".version") %>%  html_text(trim = T)`. Corrisponde a quella installata? Se sì procedi avanti. Se non corrisponde o compare qualche altro errore sentiti libero di mandarmi una mail [me medesimo](mailto:niccolo.salvini27@gmail.com). Se hai installata una versione precedente di `git` non occorre aggiornare per questo tutorial, anche se puoi considerare l'idea di farlo in prossimo futuro.
...
Uno dei vantaggi di usare uno strumento di verion control con commits regolari è poter avere la cronostoria di tutto il lavoro sul progetto. E' possibile infatti guardare l'History (la serie di commits) sia online che in locale su RStudio.
